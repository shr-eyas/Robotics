...
for i in range(trials):
    ...
    for j in range(len(timer) - 1):
        ...
        trial_positions.append(Q)

    positions.append(trial_positions)
    
    error[i] = np.array(desX[:, :len(timer) - 1]) - np.array(positions[i]).T
    print(f"i: {i}, Error: {error[i]}")
    bodyF[i + 1] = lambda_ILC * bodyF[i] + gamma_ILC * error[i]
    robot.torque_de_activate(robotIDs)

    # plt.figure()
    # x = range(len(fingerF[i][0]))  
    # plt.plot(x, fingerF[i][0], label='Finger 0')
    # plt.plot(x, fingerF[i][1], label='Finger 1')
    # plt.plot(x, fingerF[i][2], label='Finger 2')
    # plt.plot(x, fingerF[i][3], label='Finger 3')
    # plt.title(f"Finger Force for Trial {i}")
    # plt.xlabel("Sample")
    # plt.ylabel("Force")
    # plt.legend()
    # plt.show()
